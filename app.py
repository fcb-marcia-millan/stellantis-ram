...
.dash-sub { font-size: 11px; color: #b0a477; letter-spacing: 2px; text-transform: uppercase; margin-top: 4px; }
.dash-badge {
    background: rgba(0,136,204,0.12); border: 1px solid #b0a477;
    color: #b0a477; font-size: 10px; padding: 4px 12px;
...
.kpi-bar-fill { height: 100%; background: #b0a477; border-radius: 1px; }
.section-title {
...
# ── Constantes ─────────────────────────────────────────────────────────────────
SHEET_ID = "18OVXxcVy_KL4XkZj78Yjj8BD_1uHKYngd4sTMO8yblY"
GID      = "0"
...
BASE = dict(
    ...
    colorway=["#b0a477","#b0a477","#00aadd","#73bf69","#fade2a","#ff780a"],
)
...
with st.sidebar:
    ...
    <div style="text-align:center;padding:16px 0 8px 0">
      <img src="https://www.ram.com.ar/content/dam/ram/nova-home/logo-ram.svg" width="100"><br>
    </div>
    """, unsafe_allow_html=True)
    ...
    modelo_sel = st.multiselect("Modelo", modelos_opts, placeholder="Todos")
    ...
    if "Gender" in df_raw.columns:
        ...
        gender_sel = st.multiselect("Género", gender_opts, placeholder="Todos")
    else:
        gender_sel = []
...
# ══════════════════════════════════════════════════════════════════════════════
# FILTROS
# multiselect vacío = sin filtro (equivale a "Todos")
# ══════════════════════════════════════════════════════════════════════════════
df = df_raw.copy()
if modelo_sel:
...
# df_gen_filtered: para vista Género con filtro de género aplicado
df_gen_filtered = df.copy()
if gender_sel and "Gender" in df_gen_filtered.columns:
...
if pagina == "General":
    ...
    with col_anio:
        ...
        fig = go.Figure(go.Bar(
            ...
            marker_color="#b0a477", marker_line_width=0,
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        colors  = ["#b0a477","#b0a477","#00aadd","#73bf69","#fade2a","#ff780a","#e02f44","#555570"]
    ...
    with col2:
        ...
        bar_colors = ["#555570" if m == SIN_DATO else "#b0a477" for m in mod_uni["am_modelocl"]]
        fig2 = go.Figure(go.Bar(
    ...
    if "mes_compra" in df.columns and not df.empty:
        ...
        if not trend.empty:
            fig3 = px.line(
                ...
                color_discrete_sequence=["#b0a477","#b0a477","#00aadd","#73bf69",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if "cl_dir_provincia" not in df.columns:
        ...
        with col1:
            ...
            bar_colors = ["#555570" if p == SIN_DATO else "#b0a477" for p in prov_df["cl_dir_provincia"]]
            fig = go.Figure(go.Bar(
        ...
        with col2:
            if "cl_dir_localidad" in df.columns:
                loc_df = (df.groupby("cl_dir_localidad")["cl_k_cliente"]
                          ...
                          .sort_values("n", ascending=True))
                bar_colors2 = ["#555570" if l == SIN_DATO else "#b0a477" for l in loc_df["cl_dir_localidad"]]
                fig2 = go.Figure(go.Bar(
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    df_corp = df[pd.to_numeric(df["empresa"], errors="coerce") == 1].copy() if "empresa" in df.columns else pd.DataFrame()

    if df_corp.empty:
        st.warning("No se encontraron registros de empresa en los datos filtrados.")
    else:
        total_corp   = df_corp["cl_k_cliente"].nunique() if "cl_k_cliente" in df_corp.columns else len(df_corp)
        compras_corp = len(df_corp)
        pct_corp     = round(compras_corp / len(df) * 100, 1) if len(df) > 0 else 0
        ...
        <div class="kpi-grid">
          <div class="kpi-card">
            ...
            <div class="kpi-sub">Suma de DNI's únicos registrados (empresa=1)</div>
          ...
          <div class="kpi-card">
            ...
            <div class="kpi-sub">Todos los registros (empresa=1)</div>
            <div class="kpi-bar"><div class="kpi-bar-fill" style="width:75%;background:#fade2a"></div></div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">% sobre total</div>
            <div class="kpi-value" style="color:#fade2a">{pct_corp}%</div>
            <div class="kpi-sub">Compras empresa / Total compras</div>
            <div class="kpi-bar"><div class="kpi-bar-fill" style="width:{pct_corp}%;background:#fade2a"></div></div>
          </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if "mes_compra" in df_corp.columns:
            ...
            df_corp_time = df_corp.copy()
            if usar_filtro_temporal and len(fecha_rango) == 2 and "vp_f_compra" in df_corp_time.columns:
                mask = ((df_corp_time["vp_f_compra"].dt.date >= fecha_rango[0]) &
                        (df_corp_time["vp_f_compra"].dt.date <= fecha_rango[1]))
                df_corp_time = df_corp_time[mask]
            corp_trend = df_corp_time.groupby("mes_compra").size().reset_index(name="n").sort_values("mes_compra")
            if not corp_trend.empty:
                fig3 = go.Figure(go.Bar(
                    x=corp_trend["mes_compra"], y=corp_trend["n"],
        ...
        st.markdown('<p class="section-title">Resumen por modelo (empresas)</p>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if "Gender" not in df_gen.columns:
        ...
        for g in generos_reales:
            ...
            col = COLOR_MAP_GEN.get(g, "#b0a477")
            kpi_html += f"""
            <div class="kpi-card">
        ...
        with col1:
            ...
            pie_colors = [COLOR_MAP_GEN.get(g, "#b0a477") for g in gen_df["Gender"]]
            fig = go.Figure(go.Pie(
